...
public class PostsController extends HttpServlet {
	...
	private void updatePost(HttpServletRequest req, HttpServletResponse resp) {
	    ...
	    if (customerParam == null || customerParam.isEmpty()) {
	        ControllerUtil.errorMessage(req, "ID do cliente não fornecido.");
	        return;  // Opcional: Adicione um redirecionamento ou encaminhamento apropriado aqui
	    }
	    
	    Integer customerId = null;
	    try {
	        customerId = Integer.parseInt(customerParam);
	    } catch (NumberFormatException e) {
	        ControllerUtil.errorMessage(req, "ID do cliente inválido.");
	        return;  // Opcional: Adicione um redirecionamento ou encaminhamento apropriado aqui
	    }

	    Post post = loadPost(req);
	    post.setContent(postContent);
	    post.setCustomer(new Customer(customerId));

	    PostDAO dao = DAOFactory.createDAO(PostDAO.class);

	    try {
	        if (dao.update(post)) {
	            ControllerUtil.sucessMessage(req, "Serviço '" + post.getContent() + "' atualizado com sucesso.");
	        } else {
	            ControllerUtil.errorMessage(req, "Serviço '" + post.getContent() + "' não pode ser atualizado.");
	        }
	    } catch (ModelException e) {
	        // log no servidor
	        e.printStackTrace();
	...
	private void insertPost(HttpServletRequest req, HttpServletResponse resp) {
		String postContent = req.getParameter("content");
		Integer customerId = Integer.parseInt(req.getParameter("customer"));
		...
		try {
			if (dao.save(post)) {
				ControllerUtil.sucessMessage(req, "Serviço '" + post.getContent() + "' salvo com sucesso.");
			}
			else {
				ControllerUtil.errorMessage(req, "Serviço '" + post.getContent() + "' não pode ser salvo.");
			}
	...
	private void deletePost(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			if (dao.delete(p)) {
				ControllerUtil.sucessMessage(req, "Serviço '" + p.getContent() + "' deletado com sucesso.");
			}
			else {
				ControllerUtil.errorMessage(req, "Serviço '" + p.getContent() + "' não pode ser deletado.");
			}
		} catch (ModelException e) {
			// log no servidor
			e.printStackTrace();
	...
	private void listPosts(HttpServletRequest req) {
	    ...
	    try {
	    	...
	    	ControllerUtil.errorMessage(req, "Posts não foram carregados.");
	        e.printStackTrace();
	    }
